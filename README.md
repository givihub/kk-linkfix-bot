...
Telegram-бот для групп: превращает ссылки Instagram (Reels/посты), TikTok, X/Twitter и Reddit в сообщения с инлайн-видео, которое можно смотреть прямо в чате.

Поддерживаемые источники и иконки подписи: 📸 Instagram · 🎵 TikTok · 🐦 X/Twitter · 👽 Reddit. (YouTube не нужен — Telegram проигрывает его сам.)

## Как работает

1. Кто-то кидает в группу ссылку `https://www.instagram.com/reel/...` или TikTok.
2. Бот удаляет исходное сообщение.
3. Вместо него отправляет видео, а под ним подпись `🎬 Ссылка`.
...
| `REDDIT_FIX_DOMAIN` | домен фиксера Reddit (по умолчанию `rxddit.com`) |
